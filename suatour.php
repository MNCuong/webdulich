...
<body>
    ...
    exit; // Kết thúc chương trình để ngăn tiếp tục thực thi
		...
		//Tạo câu truy vấn
		$sql_select_tour="Select * from `tourdulich` WHERE `tourdulich`.`id` = $id";
		$result_se_tour=mysqli_query($conn,$sql_select_tour);
		$row=mysqli_fetch_object($result_se_tour);
		$id=$row->id;
		$diadiem=$row->diadiem;
	...
	<form name="frm_suadulich" action="Connect_suatour.php" method="post" enctype="multipart/form-data">
		<input type="hidden" name="id" value="<?php echo $id; ?>"> <!-- Trường ẩn để truyền giá trị id -->
		<table width="405" border="1" align="center">
		  <tbody>
			<tr>
			  <td colspan="2" align="center">Sửa tour du lịch</td>
			</tr>
<!--
			<tr>
			  <td width="149">ID</td>
...
-->
			<tr>
			  <td>Địa điểm</td>
			  <td><input type="text" name="txt_diadiem" value="<?php echo $diadiem?>"></td>
			</tr>
			<tr>
			  <td>Tỉnh Thành</td>
			  <td><input type="text" name="txt_tinhthanh" value="<?php echo $tinhthanh?>"></td>
			</tr>
			<tr>
			  <td>Thể Loại</td>
